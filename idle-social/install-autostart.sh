#!/bin/bash
# Fa partire IDLE SOCIAL automaticamente all'accesso al Mac (LaunchAgent).
# Uso: ./install-autostart.sh        per installare
#      ./install-autostart.sh remove per rimuovere
set -euo pipefail

LABEL="com.idle-social.server"
...
echo "✅ IDLE SOCIAL partirà da solo a ogni accesso. Log: $DIR/.data/server.log"
