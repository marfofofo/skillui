...
const tools = [
  ...
  },
  {
    ...
    input_schema: { type: "object", properties: {} },
    async run() {
      const base = path.join(os.tmpdir(), `idle-social-${Date.now()}`);
      const png = `${base}.png`;
      const jpg = `${base}.jpg`;
      try {
  ...
  },
  {
    ...
    async run(input) {
      await osascript(
        ...
        [input.title || "IDLE SOCIAL", requireString(input, "message")],
      );
      return "OK";
    },
  },
  ...
  },
  {
    ...
    async run(input) {
      ...
      const file = path.join(os.tmpdir(), `idle-social-input-${Date.now()}.txt`);
